...
```bash
bash <(curl -s -L https://raw.githubusercontent.com/YFPS/MinerProxy/main/install.sh)
```

# Windows 安装

```bash
双击打开minerProxy_web.exe 运行成功后访问 IP:18888 (如：127.0.0.1:18888 注意开放端口) 进行配置即可。
```
...
firewall-cmd --zone=public --add-port=1-30201/tcp --permanent
...
安装好 wget 之后 下载脚本并执行
```bash
wget https://raw.githubusercontent.com/YFPS/MinerProxy/main/install.sh
bash install.sh
```
...
# Liunx-手动安装
```bash
git clone https://github.com/YFPS/MinerProxy.git
cd MinerProxy
...
server:
    - port: "5555"
      ssl: 0
      proxypool: ssl://asia2.ethermine.org:14444
      devfee: 0.5
